...
//					.setFakeBoldText(true);
			...
			if (message != null) {
				...
				if(message.contains("#")){
					...
					SpannableStringBuilder builder = new SpannableStringBuilder(textView.getText().toString().replaceAll("#", ""));  
					//ForegroundColorSpan 为文字前景色，BackgroundColorSpan为文字背景色  
					ForegroundColorSpan blackSpan = new ForegroundColorSpan(Color.BLACK);  
					ForegroundColorSpan mainSpan = new ForegroundColorSpan(context.getResources().getColor(R.color.text_main));  
					builder.setSpan(blackSpan, 0, content[0].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);  
					builder.setSpan(mainSpan, content[0].length(), (content[1].length()+content[0].length()), Spannable.SPAN_INCLUSIVE_INCLUSIVE);  
					builder.setSpan(blackSpan,content[0].length()+content[1].length(), content[0].length()+content[1].length()+content[2].length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);  
